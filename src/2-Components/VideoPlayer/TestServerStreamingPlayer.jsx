...
const TestServerStreamingPlayer = ({
  ...
  // Buffer building function - only for manual buffer building
  const startBufferBuilding = useCallback((targetBufferSeconds = 5) => {
    const video = videoRef.current;
    if (!video) return;
    ...
    const checkBuffer = () => {
      ...
      if (bufferAhead >= targetBufferSeconds) {
        ...
        // Auto-play if was playing before buffer build
  ...
  }, [showBufferLoader, cleanupIntervals]);
