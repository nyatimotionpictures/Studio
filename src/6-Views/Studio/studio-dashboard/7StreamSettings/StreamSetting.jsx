...
import { createNewCategory } from "../../../../5-Store/TanstackStore/services/api";
...
const StreamSetting = () => {
  ...
  /** use mutation: create new category */
  ...
  React.useEffect(()=>{
    if(categoriesQuery?.status === "error"){
      setSnackbarMessage({ message: categoriesQuery?.error?.message, severity: "error" });
    }else {
      ...
      <div className="grid grid-cols-[auto,1fr] flex-grow-1 relative overflow-auto">
        ...
        <div className="bg-[#24222a] min-h-[100vh] flex-1 px-10 overflow-auto">
          ...
          <div className="pt-7 pb-11 ">
            <CategoryTable categories={categoriesQuery.data?.categories} />
          </div>
        </div>
      </div>
